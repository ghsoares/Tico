...
export function treefy(tree: Object, options: TreefyOptions = {}): string {
	...
	const {
		...
		nullColor = [fromHex("#FDDAEE"), fromHex(null)],
		undefinedColor = [fromHex("#FFC7CF"), fromHex(null)],
	} = options;
	 ...
	 */
	const applyColor = (str: string, c: [Color, Color]): string => colors ? colorfy(str, c[0], c[1]) : str;
	 ...
	 */
	const indent = (lvl: number): string => " ".repeat(lvl * (indentSize + 1));
...
export function throwErrorAtPos(str: string, pos: number, msg: string) {
	...
	const locationStr = lines.map((line, idx) => {
		...
		if (idx === 2) cursorOffset += lineNumber.length;
